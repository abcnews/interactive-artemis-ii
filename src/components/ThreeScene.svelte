<script lang="ts">
  ...
  let scene = $derived(whichScene({ downpage: scroll.pageScrollBottom }));

  let isSetupVisible = $derived(scene === "setup");
  let isZoomVisible = $derived(scene === "zoomsection" || scene === "outro");
</script>
...
>
  <Canvas renderMode={isSetupVisible ? "always" : "manual"}>
    <T.PerspectiveCamera
      makeDefault
...
<!-- {:else if whichScene( { downpage: scroll.pageScrollBottom }, ) === "zoomsection" || whichScene( { downpage: scroll.pageScrollBottom }, ) === "outro" || whichScene( { downpage: scroll.pageScrollBottom }, ) === "setup"} -->
<div
  ...
  class:faded={whichScene({ downpage: scroll.pageScrollBottom }) === "outro" ||
    whichScene({ downpage: scroll.pageScrollBottom }) === "setup"}
>
  {#if gpu.qualityTier === "low"}
    <StarfieldStatic />
  {/if}
  <Canvas
    renderMode={isZoomVisible ? "always" : "manual"}
